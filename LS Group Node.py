...
_Learning Studio Flavour: Has a "Grouped Nodes" power - like a custom power, but you just need to tick the box in config to add it_
'''
...
@local_action({'group': '"Power"', 'title': 'Group Power', 'schema': {'type': 'string', 'enum': ['On', 'Off']}, 'order': next_seq()})
def groupNodes(arg):
    for node in groupedNodes:
      # console.log(node)
      lookup_remote_action(node).call(arg)
...
param_members = Parameter({'title': 'Members', 'schema': {'type': 'array', 'items': {'type': 'object', 'properties': {
   ...
   'power': {'title': 'Power', 'type': 'object', 'order': 4, 'properties': {
     ...
     'grouped': {'title': "Include in 'GroupOn/GroupOff' action?", 'type': 'boolean'}
   }},
...
def initMember(memberInfo):
  ...
  isGroupedNode = memberInfo.get('power').get('grouped')

  if (memberInfo.get('power') or {}).get('mode') in MODES:
    initSignalSupport(name, memberInfo['power']['mode'], 'Power', ['On', 'Off'], disappears, isGroup, isGroupedNode)
    
  if (memberInfo.get('muting') or {}).get('mode') in MODES:
    initSignalSupport(name, memberInfo['muting']['mode'], 'Muting', ['On', 'Off'], disappears, isGroup, isGroupedNode)
...
def initSignalSupport(name, mode, signalName, states, disappears, isGroup, grouped):
  ...
  if grouped:
    groupedNodes.append('Member %s %s' % (name, signalName))
