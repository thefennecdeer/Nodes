'''
##### **App Node (with Wait Application)** 

___

Based on regular app node - but initally starts a different executable (SET IN NODE CONFIG).

* The node will wait until the action 'MainStart' is triggered, either locally or remotely.

* Upon recieving this trigger, the wait application will remain open for ~20 seconds, so we dont flash the desktop while the actual program loads.

_Use the `Jump Controls` group of actions to trigger the 'MainStart' manually!_
...
param_WaitPath = Parameter({'title': 'Waiting App Path (required, executable name with or without path)', 'required': True, 'schema': {'type': 'string', 'hint': '(e.g. "C:\\MyApps\\myapp.exe" or "somethingOnThePath.exe")'},
                           'desc': 'The full path to the application executable that runs before the VR is loaded'})
...
_resolvedWaitAppPath = None # includes entire path
...
def main():
  ...
  if is_blank(param_AppPath):
    ...
    _process.stop()
    return
  if is_blank(param_WaitPath):
    console.error('No Waiting App. Path has been specified, nothing to do!')
    _process.stop()
    return
  ...
  if os.path.sep in param_AppPath: # e.g. 
    ...
    finishMain()
  if os.path.sep in param_WaitPath: # e.g. 
    global _resolvedWaitAppPath
    _resolvedWaitAppPath = param_WaitPath # use full path
    finishMain()
...
def finishMain():
  ...
  cmdWaitLine = [_resolvedWaitAppPath]
  ...
  _waitProcess.setCommand(cmdWaitLine)
...
@local_action({'group': 'Jump', 'title': 'Start Wait Application', 'order': next_seq()})
def WaitStart():
  console.log("Launching Wait Screen")
  _waitProcess.start()
  
@local_action({'group': 'Jump', 'title': 'Start Main Application', 'order': next_seq()})
def MainStart():
  global waiting
  console.log("Launching Main Application")
  waiting = False
  _process.start()  
  call(lambda: _waitProcess.stop(),20) #delay closing Wait screen so we arent shown the desktop inbetween loading
